...
BEGIN
    ...
    request.method := 'POST';
    request.uri := supabase_url || '/functions/v1/contact-notification';
    request.headers := jsonb_build_object(
        'Authorization', 'Bearer ' || service_role_key,
        'Content-Type', 'application/json'
    );
    request.content := jsonb_build_object('record', row_to_json(NEW))::text;
    ...
    INSERT INTO contact_request_logs (
        ...
        CASE 
            WHEN response.status BETWEEN 200 AND 299 THEN 'success'
            ELSE 'failed'
        END,
        CASE 
            WHEN response.status BETWEEN 200 AND 299 THEN 'Successfully called notification function'
