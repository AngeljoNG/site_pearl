...
// Maximum characters for localStorage (approximately 500KB)
...
export function BlogAdmin() {
  ...
  useEffect(() => {
    ...
    if (!editingPost) {
      ...
      const savedExcerpt = localStorage.getItem('blogDraftExcerpt');
      const savedContent = localStorage.getItem('blogDraftContent');
  ...
  useEffect(() => {
    ...
    const autoSaveInterval = setInterval(() => {
      if (title || excerpt || editorContent) {
        try {
          // Check content length before saving
          if (editorContent.length > MAX_CONTENT_LENGTH) {
            setAutoSaveStatus('⚠️ Contenu trop volumineux pour la sauvegarde automatique');
            return;
          }

          localStorage.setItem('blogDraftTitle', title);
          localStorage.setItem('blogDraftExcerpt', excerpt);
          localStorage.setItem('blogDraftContent', editorContent);
          setAutoSaveStatus('Brouillon sauvegardé');
          setTimeout(() => setAutoSaveStatus(''), 2000);
        } catch (error) {
          console.error('Error saving draft:', error);
          setAutoSaveStatus('⚠️ Erreur lors de la sauvegarde automatique');
        }
      }
    }, 30000);

    return () => clearInterval(autoSaveInterval);
  }, [title, excerpt, editorContent, editingPost]);
  ...
  const handleEdit = (post: any) => {
    ...
    setImageSize({ width: post.image_width, height: post.image_height });
  };
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3">
          <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow-md">
            ...
            {!editingPost && autoSaveStatus && (
              <div className={`text-sm italic ${autoSaveStatus.includes('⚠️') ? 'text-amber-500' : 'text-nature-500'}`}>
                {autoSaveStatus}
              </div>
            )}
            ...
            <div>
              ...
              {editorContent.length > MAX_CONTENT_LENGTH && (
                <p className="text-sm text-amber-500 mt-2">
                  ⚠️ Le contenu dépasse la limite de caractères pour la sauvegarde automatique. Certaines modifications pourraient ne pas être sauvegardées.
                </p>
              )}
            </div>
